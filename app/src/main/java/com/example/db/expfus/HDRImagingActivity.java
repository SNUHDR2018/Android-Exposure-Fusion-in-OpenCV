...
public class HDRImagingActivity extends AppCompatActivity {
    ...
    private Mat exposureFusion (List<Mat> images, double w_c, double w_s, double w_e, int depth)
    {
        ...
        {
            Mat ls = Mat.zeros(lps.get(0).get(lev).size(),CV_8UC4);
            for(int i=0;i<N;i++)
            {
                Mat lp = lps.get(i).get(lev);
                lp.convertTo(lp,CV_32FC4);
                ...
                gpList.add(gpsFloat);
                gpList.add(gpsOnes);
                Mat gp = new Mat(gpsFloat.size(),CV_32FC4);
                Core.merge(gpList,gp);
                Mat lp_gp = new Mat(lp.size(),CV_8UC4);
                Core.multiply(lp,gp,lp_gp,255,CV_8UC4);
        ...
        {
            Imgproc.pyrUp(resultImage, resultImage, new Size((double) resultImage.width() * 2, (double) resultImage.height() * 2));
    ...
    private List<Mat> laplacianPyramid(Mat mat,int depth)
    {
        ...
        for(int i=depth-1;i>=1;i--)
        {
            ...
            Mat dst = new Mat(m.width()*2,m.height()*2,m.type());
            Imgproc.pyrUp(m, dst, new Size((double) m.width() * 2, (double) m.height() * 2));
    ...
    private List<Mat> computeWeights (List<Mat> images, double w_c, double w_s, double w_e) {
        ...
        for(int i = 0; i < images.size(); i++)
        {
            ...
            //CONTRAST
            Mat imgGray = new Mat(r,c,CV_32FC1);
            Imgproc.cvtColor(imgFloat,imgGray,Imgproc.COLOR_RGB2GRAY);
            Mat imgLaplacian = new Mat(r,c,CV_32FC1);
            Imgproc.Laplacian(imgGray,imgLaplacian,CV_32FC1);
            ...
            Core.add(std,G,std);
            Core.add(std,B,std);
            Core.divide(std,Scalar.all(2),std);
